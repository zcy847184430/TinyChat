...
/*预定义数据库名称*/
#define DB_NAME "../TinyChat/chatRome.db"
...
typedef struct _Message{
	...
	int msgType;			//消息类型
	int msgRet;				//消息结果
	struct sockaddr_in sendAddr;  //发送方地址
	struct sockaddr_in recvAddr;	//接收方地址
	char sendName[20]; 		//发送方用户名
	char recvName[20]; 		//接受方用户名
	char msgTime[20];  		//发送时间
}Message;
